...
class CleanMesh(object):
    ...
    def __compact_vertices(self, points: np.ndarray, triangles: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def clean_mesh(
        ...
        triangles_clean, cell_data_clean, removed_duplicate = self.__remove_duplicate_faces(triangles_clean, cell_data_clean)

        # 4) Compact vertex list to used vertices only
        points_clean, triangles_clean = self.__compact_vertices(points_merged, triangles_clean)
    ...
    def __print_stats(self, label: str, s: MeshStats) -> None:
        print(f"{label}")
